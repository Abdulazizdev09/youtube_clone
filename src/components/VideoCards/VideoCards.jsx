...
const VideoCards = () => {
    const onRender = (id, phase, actualDuration, baseDuration, startTime, commitTime) => {
        console.log(`%c ${startTime} - ${commitTime}`, "color:green;font-weight:900;font-size:20px")
    }
    ...
    return (
        <VideosModal>
            <Profiler id='VideoProfiler' onRender={onRender}>
                <div className="grid mt-[10px] ml-[60px] grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-4 p-4 bg-black">
                    {loading
                        ? <CircularProgress />
                        : videos.map((video) => (
